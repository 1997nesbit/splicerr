<script lang="ts">
    ...
    import { openBrowse, openCollection, openPackSearch, view } from "$lib/shared/view.svelte"
    import { packSearch, fetchPopularPacks } from "$lib/shared/store.svelte"
    ...
    const confirmDelete = () => {
        ...
        if (
            ...
            view.collection?.uuid === deleteTarget.uuid
        ) {
            openBrowse()
        }
    ...
    <div class="flex flex-col gap-0.5">
        ...
        <button
            class={cn(
                ...
                view.kind === "browse" && "bg-muted font-medium"
            )}
            onclick={openBrowse}
        >
        ...
        <button
            class={cn(
                "flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-left w-full hover:bg-muted",
                view.kind === "packs" && "bg-muted font-medium"
            )}
            onclick={() => {
                openPackSearch()
        ...
        >
            <Layers size="16" class="flex-shrink-0" />
            Packs
        </button>
    </div>
        ...
        <ScrollArea class="flex-grow -mx-1">
            <div class="flex flex-col gap-0.5 px-1">
                {#if likesCollection()}
                    {@const likesActive =
                        view.kind === "collection" &&
                        view.collection?.uuid === LIKES_UUID}
                    <div
                        class={cn(
                ...
                {@const active =
                    view.kind === "collection" &&
                    view.collection?.uuid === collection.uuid}
                <div
                    class={cn(
